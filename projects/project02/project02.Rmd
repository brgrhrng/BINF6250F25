...
def build_markov_model(markov_model, text, order=1):
    ...
    #TODO: error out if given order higher than string length
    
    # Extract words, separated by spaces
    list_of_words = text.split()
    
    # Create a fake start state, with a 1-count transition right to the first word.
    # initalize a sub-dict for *S* if we don't yet have one 
    markov_model["*S*"] = markov_model.get("*S*", dict())

    # For higher-order markov chains, we need a multi-word key linking word #2
    #   back to the start.
    if order > 1:
        list_of_words.insert(0,"*S*")
        next_word = " ".join(list_of_words[1:order+1])
        inner_dict = markov_model.get("*S*", dict)
        inner_dict[next_word] = inner_dict.get(next_word,0) + 1
        markov_model["*S*"] = inner_dict
     
    # Since our keys are always 1 word, the end-state can just be appended as a "word"
    list_of_words.append("*E*")
    
    # NOW THAT WE HAVE the true list, get the length
    length = len(list_of_words)
    
    # loop through all n-length spans of words (ending before our "window" reaches out of bounds)
    for i in range(1, length-order):
...
#markov_model = build_markov_model(markov_model, text, order=4)
#markov_model
