...
def build_markov_model(markov_model, new_text):
    ...
    for word in list_of_words:
        if (index+1) < length:
            ...
            if test == "Not present":
                markov_model[word] = { list_of_words[index+1]:1 }
            else:
                print(list_of_words[index+1],index+1)
                markov_model[word].setdefault( list_of_words[index+1]:1 )
              #  test2 = markov_model[word].get(list_of_words[index+1],"not in second dictionary")
              #  if test == "not in second dictionary":
              #      markov_model[word].setdefault( list_of_words[index+1]:1 )
              #  else:
              #      markov_model[word].setdefault( list_of_words[index+1]:1 )
            index+=1
        else:
            markov_model[word][index] = { list_of_words[index]:1 }
            markov_model[word][index] = { "*E*":1 }# end state
