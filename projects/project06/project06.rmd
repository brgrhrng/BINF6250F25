...
def neighbor_joining(
    ...
    if n==2:
      ...
      return f"({newick_str}:{length_from_root},{labels[0]}:{length_from_root})"
    
    # Step 1: From distance matrix, calculate total distances of each sequence
    total_distance = distance_matrix.sum(axis = 0)
    total_distance = [val/(n-2) for val in total_distance]
    
    print("-------------------------------------")
    print("DISTANCE MATRIX:\n", distance_matrix)
    print("DISTANCE TOTALS:\n", total_distance)
    
    # Step 2: Create Q matrix (D prime). For non-symmetric comparisons:
    #    D*ij = Dij - TotalDistance(i) - TotalDistance(j)
    #   Symmetric comparisons are always 0.
    ...
    for i, row in enumerate(distance_matrix):
      for j, distance in enumerate(row):
        if i == j:
          ...
          q_matrix[i,j] = distance - total_distance[i] - total_distance[j]
          q_matrix[j,i] = q_matrix[i,j] # mirror values across diagonal
        
    print("Q MATRIX:\n", q_matrix)
    
    # Step 3: Join two nodes and calculate branch lengths
    
    # Find a minimum value in D', and the two sequences share it 
    #   i.e. both [i,j] and [j,i] contain the min
    ...
    delta = (total_distj - total_disti)
    
    # calc limb lengths
    limb_lengthi = (distance_matrix[i,j] - delta) / 2
    limb_lengthj = (distance_matrix[i,j] + delta) / 2
    
    print(limb_lengthi,limb_lengthj)
    ...
    for row_i, row in enumerate(distance_matrix):
      merged_val = (row[i] + row[j] - distance_matrix[i,j]) / 2
      merged_vals[row_i] = merged_val
      
    print("merged_vals!!!", merged_vals)
    
    # Replace species i with merged
    distance_matrix[:, i] = merged_vals
    distance_matrix[i] = merged_vals
    
    labels[i] = f"{labels[i]},{labels[j]}" #
    labels.pop(j)
    ...
    (D:10.0,C:4.0)
    (A:13,(D:10.0,C:4.0):4)
    ((A:13,(D:10.0,C:4.0):4),B:4) # final step
    
    (A:13.0,(D:10.0,C:4.0):4.0):4)
    ...
    newick = f"({lbl_i}:{limb_lengthi},{newick_str}:{limb_lengthj})"
    ...
    Tree("(A:1,(B:1,(E:1,D:1):0.5):0.5);")
    
    
    D:10,(C:4,(A:4,B:13):4.0):10
