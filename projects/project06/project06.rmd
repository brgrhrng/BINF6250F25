...
def get_min_distance(matrix):
    ...
    possible_positions = np.where(matrix == minimum)
    # Create a list of tuples containing the coordinates to possible minimums.
    choices = list(zip(possible_positions[0], possible_positions[1]))
    # Randomly choose an index from that list. 
    choice_index = np.random.choice(len(choices))
    # Get corresponding coordinates.
    min_pos = choices[choice_index]
...
def neighbor_joining(
    ...
    if n==2:
      ...
      if len(labels[0]) > 1 and len(labels[1]) > 1:
        return f"{newick_str}:{length_from_root}"
      elif len(labels[0]) == 1:
        final_taxa = labels[0]
      else:
        final_taxa = labels[1]
      return f"({newick_str}:{length_from_root},{final_taxa}:{length_from_root})"
    ...
    elif id_1[0] not in newick_str and id_2[0] not in newick_str:
    ...
    # Attach unmerged taxa to merged taxa
      ...
      newick = f"({id_1}:{limb_length1},{newick_str}:{limb_length2})"
    else:
