...
class FMIndex:
  """
  An object that stores a compressed text sequence, with helper functions for efficient string matching based on the Burrows-Wheeler transform.
  Only the compressed BW-T of the reference string, and some extra derived data structures are stored. The original sequence can be recovered  
  TODO: store BWT in run-length compressed form, and expand it with a property.
  Make initial sequence recoverable with either an explicit getter, or a property.
  """
  end_char = "$"
  
  def __init__(self, reference_text: str):
    # Store counts dict for reference
    self._ref_char_counts = self._count_chars(reference_text)
    ...
    self.bwt = BWT_fast(reference_text) # TODO: run-length encoding?
    
    # Store occurrences array (dict) for BWT
    self._bwt_occurence_map = self._occurences_map(self.bwt)
    
  
  def _count_chars(self, ref_string):
    """Generate a dict of (char, count) tuples for all chars in alphabet, 
    where "count" is the number of characters in ref_string lexicographically
    smaller than char."""
