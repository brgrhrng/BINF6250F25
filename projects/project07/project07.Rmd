...
class FMIndex:
  ...
  def locate(self, query: str) -> list[int]:
    ...
    hits = [self.suffix_indexes[i] for i in range(lower,upper+1)]
    return sorted(hits)
  ...
  def _update_search_window(self, char: str, upper: int, lower: int):
    ...
    if lower == 0:
      ...
      lower_new = lesser_char_count + self._bwt_occurence_map[char][lower-1] + 1
    upper_new = lesser_char_count + self._bwt_occurence_map[char][upper] - 1
    
    return lower_new, upper_new
    
    
test1 = FMIndex("TACATABATACATACATACB")

print(test1.bwt)
print(test1.get_reference_string())
print(test1.count("TAC"))
print(test1.suffix_indexes)
print(test1.locate("TAC"))
