...
def GibbsMotifFinder(seqs, k, seed=None):
    ...
    # For each sequence, initialize the motif
    #   as a random substring of length k
    motifs_i = [] # start positions
    ...
    # Each loop, we will compare the new PFM's
    # info content to see if it's still changing
    previous_IC = 0.0
    
    # Main loop
    for iterations in range(0,1000):
        # Randomly select a sequence to update
        ...
        # Score each k-length sub-string of chosen sequence with PWM
        ...
        # Check if the PFM's info content has plateaud
...
def score_substrings(seq, k, pwm):
  """
  Function to score all k-length substring of seq using pwm.
  Normalizes all scores to the (0,1) range, and returns in a list.
  """
    scores = []
    relative_scores = []
    
    # Extract each k-mer and score it
